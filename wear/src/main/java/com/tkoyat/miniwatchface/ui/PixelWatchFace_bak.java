...
import com.tkoyat.miniwatchface.util.WatchFaceUtil;
import com.tkoyat.miniwatchface.workers.LocationUpdateWorker;
import com.tkoyat.miniwatchface.workers.WeatherUpdateWorker;
...
import static com.tkoyat.miniwatchface.util.Constants.NO_TEMPERATURE;
import static com.tkoyat.miniwatchface.util.Constants.WEATHER_BACKOFF_DELAY_ONETIME;
import static com.tkoyat.miniwatchface.util.Constants.WEATHER_ICON_MARGIN_RATIO;
import static com.tkoyat.miniwatchface.util.Constants.WEATHER_ICON_Y_OFFSET_RATIO;
import static com.tkoyat.miniwatchface.util.Constants.WEATHER_UPDATE_INTERVAL;
import static com.tkoyat.miniwatchface.util.Constants.WEATHER_UPDATE_WORKER;
import static com.tkoyat.miniwatchface.util.WatchFaceUtil.drawableToBitmap;


public class PixelWatchFace_bak extends CanvasWatchFaceService {
  ...
  private static class EngineHandler extends Handler {
    ...
    private final WeakReference<PixelWatchFace_bak.Engine> mWeakReference;

    public EngineHandler(PixelWatchFace_bak.Engine reference) {
      mWeakReference = new WeakReference<>(reference);
    }

    @Override
    public void handleMessage(Message msg) {
      PixelWatchFace_bak.Engine engine = mWeakReference.get();
      if (engine != null) {
        switch (msg.what) {
...
//          new ComponentName("com.google.android.wearable.app", WEATHER_PROVIDER_SERVICE), ComplicationData.TYPE_ICON);
      ...
      setWatchFaceStyle(new WatchFaceStyle.Builder(PixelWatchFace_bak.this)
...
//      }
    ...
    public void onApplyWindowInsets(WindowInsets insets) {
      ...
      Resources resources = PixelWatchFace_bak.this.getResources();
      ...
      for (DataEvent event : dataEvents) {
        if (event.getType() == DataEvent.TYPE_CHANGED) {
          ...
          if (item.getUri().getPath().compareTo("/settings") == 0) {
            ...
            if (dataMap.containsKey("com.tkoyat.miniwatchface")) {
              dataMap = dataMap.getDataMap("com.tkoyat.miniwatchface");
            }
